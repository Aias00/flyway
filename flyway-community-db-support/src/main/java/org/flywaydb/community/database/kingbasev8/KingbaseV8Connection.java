...
/**
 * KingbaseES connection.
 */
public class KingbaseV8Connection extends Connection<KingbaseV8Database> {
    private final String originalRole;

    protected KingbaseV8Connection(KingbaseV8Database database, java.sql.Connection connection) {
    ...
    public Schema getSchema(String name) {
        return new KingbaseV8Schema(jdbcTemplate, database, name);
    }

    @Override
    public <T> T lock(Table table, Callable<T> callable) {
        return new KingbaseV8AdvisoryLockTemplate(jdbcTemplate, table.toString().hashCode()).execute(callable);
    }
}